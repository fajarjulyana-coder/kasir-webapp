class POSSystem {
  ...
  async searchItems(query) {
    ...
    try {
      const response = await fetch(
        `/cashier/search_item?q=${encodeURIComponent(query)}`
      );
        ...
        .map(
          (item) => `
                <div class="col-md-6 col-lg-4">
                    <div class="card item-card" data-item='${JSON.stringify(
                      item
                    )}'>
                        <div class="card-body p-3">
                            ...
                            <p class="card-text">
                                ...
                                <strong>Rp ${item.harga_jual.toLocaleString(
                                  "id-ID"
                                )}</strong>
                            </p>
  ...
  async addItemByCode(itemCode) {
    try {
      ...
      const response = await fetch(
        `/cashier/search_item?q=${encodeURIComponent(itemCode)}`
      );
      const items = await response.json();

      // Find exact match by code
      const item = items.find(
        (i) => i.kode.toUpperCase() === itemCode.toUpperCase()
      );
      ...
      if (item) {
        ...
        this.showNotification(
          `Barang dengan kode "${itemCode}" tidak ditemukan atau stok habis`,
          "warning"
        );
  ...
  addToCart(item) {
    const existingItem = this.cart.find(
      (cartItem) => cartItem.kode === item.kode
    );
        ...
        return `
                <div class="cart-item">
                    <div class="cart-item-header">
                        ...
                        <button class="remove-item" data-kode="${
                          item.kode
                        }" title="Hapus item">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <span>Rp ${item.harga_jual.toLocaleString(
                          "id-ID"
                        )}</span>
                        <span><strong>Rp ${subtotal.toLocaleString(
                          "id-ID"
                        )}</strong></span>
                    </div>
                    <div class="cart-item-controls">
                        <div class="quantity-control">
                            <button class="quantity-btn quantity-decrease" data-kode="${
                              item.kode
                            }">
                                <i class="fas fa-minus"></i>
                            </button>
                            <input type="number" class="quantity-input" data-kode="${
                              item.kode
                            }" 
                                   value="${item.quantity}" min="1" max="${
          item.stok_tersedia
        }">
                            <button class="quantity-btn quantity-increase" data-kode="${
                              item.kode
                            }">
  ...
  calculateChange() {
    ...
    if (paymentAmount > 0 && changeDisplay) {
      if (change >= 0) {
        ...
        alertDiv.innerHTML = `<strong>Kembalian: Rp ${change.toLocaleString(
          "id-ID"
        )}</strong>`;
      } else {
        changeDisplay.className = "mb-2";
        ...
        alertDiv.innerHTML = `<strong>Kurang: Rp ${Math.abs(
          change
    ...
    if (summaryContainer) {
      summaryContainer.innerHTML = `
                <div class="text-start">
                    ...
                    <p><strong>Total:</strong> Rp ${result.total.toLocaleString(
                      "id-ID"
                    )}</p>
                    <p><strong>Bayar:</strong> Rp ${(
...
`;
    ...
    <div>
        ...
        <div style="display: flex; justify-content: space-between;">
            <span>${item.quantity} x Rp ${item.harga_jual.toLocaleString(
        "id-ID"
      )}</span>
            <span>Rp ${(item.quantity * item.harga_jual).toLocaleString(
              "id-ID"
            )}</span>
        </div>
    </div>
...
<div style="margin-top: 10px;">
    ...
    <div style="display: flex; justify-content: space-between; margin-top: 10px;">
        <span>Bayar:</span>
        <span>Rp ${(result.payment_amount || result.total).toLocaleString(
          "id-ID"
        )}</span>
    </div>
    ...
    printWindow.document.write(`
            <html>
                <head>
                    ...
                    <style>
                        @page {
                            size: 80mm auto;   /* ukuran thermal roll */
                            margin: 0;
                        }
                        body { 
    ...
    pdfWindow.document.write(`
            <html>
                <head>
                    ...
                    <style>
                        @page {
                            size: 80mm auto;  /* untuk PDF thermal roll */
                            margin: 5mm;
                        }
                        body { 
                            font-family: 'Courier New', monospace;
                ...
                <body>
                    ...
                    ${receiptContent}
                    <br><br>
                    <p><small>Generated on ${new Date().toLocaleString(
                      "id-ID"
                    )}</small></p>
